...
CREATE OR REPLACE PACKAGE BODY PCK_JUECES IS
    ...
    BEGIN
        ...
        WHILE DIAS < P_CANT_DIAS LOOP
            V_FECHA_AUX:= P_FECHA_INICIO + DIAS;
            SELECT COUNT(*) INTO V_FECHA_FERIADO FROM FERIADOS f WHERE TO_CHAR(TRUNC(f.DIA_FERIADO), 'DD/MM/YYYY') = TO_CHAR(TRUNC(V_FECHA_AUX), 'DD/MM/YYYY');
            V_DIA_SEMANA := TO_CHAR(V_FECHA_AUX, 'DAY');
            IF V_DIA_SEMANA <> 'SATURDAY' AND V_DIA_SEMANA <> 'SUNDAY' AND V_FECHA_FERIADO = 0 THEN
                DIAS := DIAS+1;
            END IF;
